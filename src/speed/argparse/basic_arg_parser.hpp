...
public:
    ...
    )
    {
        ...
        for (auto& x : kys)
        {
            // TODO(killian.poulaud@etu.upmc.fr): Check this if.
                    ...
                    if (!err_id_.empty())
                    {
                        if (flgs_.is_set(arg_parser_flags::USE_COLORS_WHEN_PRINT_ERRORS))
                        {
                            speed::system::set_terminal_text_attribute(
                                    std::cout, speed::system::terminal_text_attribute::LIGHT_RED);
                            std::cout << err_id_ << ": ";
                            speed::system::set_terminal_text_attribute(
                                    std::cout, speed::system::terminal_text_attribute::DEFAULT);
                        }
                        else
                        {
                            std::cout << err_id_ << ": ";
