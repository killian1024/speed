...
/**
 * @file        speed/system/terminal.hpp
 * @brief       terminal functions header.
 * @author      Killian
 * @date        2017/10/18 - 01:08
 */

#ifndef SPEED_SYSTEM_TERMINAL_HPP
#define SPEED_SYSTEM_TERMINAL_HPP

#include <cstdio>
#include <iostream>

#include "data_types.hpp"
#include "glibc/terminal.hpp"
...
{
    return SPEED_SELECT_API(flush_input_terminal, false, fd, err_code);
}
...
{
    return SPEED_SELECT_API(flush_output_terminal, false, fd, err_code);
}
...
{
    return SPEED_SELECT_API(kbhit, false, mess, flush_input_term, err_code);
}
 ...
 */
inline bool set_terminal_text_attribute(
        ::FILE* strm,
        terminal_text_attribute term_text_attribute
) noexcept
{
    return SPEED_SELECT_API(set_terminal_text_attribute, false, strm, term_text_attribute);
}
...
{
    return SPEED_SELECT_API(set_terminal_text_attribute, false, os, term_text_attribute);
}
