...
/**
 * @file        speed/system/glibc/process.cpp
...
alloc_fail:
        ...
        default:
        {
            ...
            {
                // TODO(killian.poulaud@etu.upmc.fr): Is that a good idea ?
                if (errno != EINTR)
                {
                    assign_system_error_code(errno, err_code);
                    return false;
...
) noexcept
{
    ...
    if (rem_time != nullptr)
    {
        rm_tm.tv_sec = rem_time->sec;
        rm_tm.tv_nsec = rem_time->nsec;
    }
    ...
    if (::nanosleep(&tm, &rm_tm) == -1)
    {
        ...
        if (rem_time != nullptr)
        {
            rem_time->sec = (std::uint64_t)rm_tm.tv_sec;
            rem_time->nsec = (std::uint64_t)rm_tm.tv_nsec;
        }
