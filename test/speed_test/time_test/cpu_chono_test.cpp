...
/**
 * @file        speed_test/system_test/cpu_chono_test.cpp
 * @brief       cpu_chono unit test.
 * @author      Killian
 * @date        2018/06/06 - 09:35
 */

#include <cmath>

#include "gtest/gtest.h"
#include "speed/time.hpp"
...
TEST(time_cpu_chono, start)
{
    ...
    for (i = static_cast<std::size_t>(pow(10, 7)); i > 0; --i)
    {
    }
    auto tme = cpu_chrn.get_elapsed_raw_time();
...
TEST(time__cpu_chono, get_elapsed_time)
{
    speed::time::cpu_chrono cpu_chrn;
    std::size_t i;
...
TEST(time__cpu_chono, get_elapsed_raw_time)
{
    speed::time::cpu_chrono cpu_chrn;
    std::size_t i;
